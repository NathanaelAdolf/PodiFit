...
class NotificationHelper: UIViewController {
    ...
    func retrieveNotificationFromCoreData(completionHandler: @escaping ([Reminder])-> Void)
    {
        ...
        do {
            ...
            guard let reminderModel = result as? [Reminder] else {return}
            completionHandler(reminderModel)
        }
        catch
        {
            print("Failed")
